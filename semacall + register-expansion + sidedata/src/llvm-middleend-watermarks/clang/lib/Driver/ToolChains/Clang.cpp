...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_watermark_atoi_key_EQ)) {
      CmdArgs.push_back(Args.MakeArgString(Twine("--atoi-key=")+A->getValue()));
  }
  if (const Arg *A = Args.getLastArg(options::OPT_watermark_atoi_val_EQ)) {
      CmdArgs.push_back(Args.MakeArgString(Twine("--atoi-val=")+A->getValue()));
  }
  if (const Arg *A = Args.getLastArg(options::OPT_watermark_gets_key_EQ)) {
      CmdArgs.push_back(Args.MakeArgString(Twine("--gets-key=")+A->getValue()));
  }
  if (const Arg *A = Args.getLastArg(options::OPT_watermark_gets_val_EQ)) {
      CmdArgs.push_back(Args.MakeArgString(Twine("--gets-val=")+A->getValue()));
  }
  if (const Arg *A = Args.getLastArg(options::OPT_watermark_time_key_EQ)) {
      CmdArgs.push_back(Args.MakeArgString(Twine("--time-key=")+A->getValue()));
  }
  if (const Arg *A = Args.getLastArg(options::OPT_watermark_time_val_EQ)) {
      CmdArgs.push_back(Args.MakeArgString(Twine("--time-val=")+A->getValue()));
  }
  if (const Arg *A = Args.getLastArg(options::OPT_watermark_val_EQ)) {
      CmdArgs.push_back(Args.MakeArgString(Twine("--watermark-val=")+A->getValue()));
  }
  if (const Arg *A = Args.getLastArg(options::OPT_watermark_key_EQ)) {
      CmdArgs.push_back(Args.MakeArgString(Twine("--watermark-key=")+A->getValue()));
  }
  if (const Arg *A = Args.getLastArg(options::OPT_sidedata_key_EQ)) {
      CmdArgs.push_back(Args.MakeArgString(Twine("--sidedata-key=")+A->getValue()));
  }
  if (const Arg *A = Args.getLastArg(options::OPT_sidedata_val_EQ)) {
      CmdArgs.push_back(Args.MakeArgString(Twine("--sidedata-val=")+A->getValue()));
  }
  if (IsCuda || IsHIP) {
