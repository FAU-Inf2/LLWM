...
#include "llvm/Support/Extension.def"
  ...
  if (!CodeGenOpts.DisableLLVMPasses) {
    ...
    GetsPatcher::GetsWaterMarkKey.setValue(CodeGenOpts.GetsKey);
    GetsPatcher::GetsWaterMarkVal.setValue(CodeGenOpts.GetsVal);
    AtoiPatcher::AtoiWaterMarkKey.setValue(CodeGenOpts.AtoiKey);
    AtoiPatcher::AtoiWaterMarkVal.setValue(CodeGenOpts.AtoiVal);
    TimePatcher::TimeWaterMarkKey.setValue(atol(CodeGenOpts.TimeKey.c_str()));
    TimePatcher::TimeWaterMarkVal.setValue(CodeGenOpts.TimeVal);
    SidedataWatermark::SidedataKey.setValue(CodeGenOpts.SidedataKey);
    SidedataWatermark::SidedataVal.setValue(CodeGenOpts.SidedataVal);
    // what you can see here is guessing how to start the function pass
    PB.registerOptimizerLastEPCallback(
        [](ModulePassManager &MPM, OptimizationLevel Level) {
          MPM.addPass(createModuleToFunctionPassAdaptor(SemaCall()));
        });
    RegisterExpansion::WaterMarkKey.setValue(CodeGenOpts.RegsKey);
    RegisterExpansion::WaterMarkVal.setValue(CodeGenOpts.RegsVal);
    PB.registerOptimizerLastEPCallback(
        [](ModulePassManager &MPM, OptimizationLevel Level) {
          MPM.addPass(createModuleToFunctionPassAdaptor(RegisterExpansion()));
        });
    PB.registerOptimizerLastEPCallback(
        [](ModulePassManager &MPM, OptimizationLevel Level) {
          MPM.addPass(createModuleToFunctionPassAdaptor(SidedataWatermark()));
        });
