; ModuleID = 'binarysearchdir/binarysearch-Os-inserted.ll'
...
.loopexit23:                                      ; preds = %.loopexit23.backedge, %2
  %3 = phi i64 [ 0, %2 ], [ %32, %.loopexit23.backedge ]
  ...
  %16 = icmp sgt i32 %controle2, 1
  br i1 %16, label %.lr.ph28, label %.loopexit

thread-pre-split:                                 ; preds = %.lr.ph28
  %17 = icmp sgt i32 %storemerge8, 1
  br i1 %17, label %.lr.ph28, label %.loopexit

.lr.ph28:                                         ; preds = %.loopexit23, %thread-pre-split
  %18 = phi i32 [ %storemerge8, %thread-pre-split ], [ %controle2, %.loopexit23 ]
  %19 = and i32 %18, 1
  %20 = icmp eq i32 %19, 0
  %21 = lshr i32 %18, 1
  %22 = mul i32 %18, 3
...
.loopexit:                                        ; preds = %.loopexit23, %thread-pre-split, %thread-pre-split18, %thread-pre-split20, %49, %65
  ret i32 0
...
28:                                               ; preds = %.lr.ph28
  ...
  %36 = icmp sgt i32 %controle6, 1
  br i1 %36, label %.lr.ph31, label %.loopexit23.backedge

.loopexit23.backedge:                             ; preds = %thread-pre-split16, %28
  br label %.loopexit23, !dbg !134

thread-pre-split16:                               ; preds = %.lr.ph31
  %37 = icmp sgt i32 %storemerge7, 1
  br i1 %37, label %.lr.ph31, label %.loopexit23.backedge

.lr.ph31:                                         ; preds = %28, %thread-pre-split16
  %38 = phi i32 [ %storemerge7, %thread-pre-split16 ], [ %controle6, %28 ]
  %39 = and i32 %38, 1
  %40 = icmp eq i32 %39, 0
  %41 = lshr i32 %38, 1
  %42 = mul i32 %38, 3
...
49:                                               ; preds = %.lr.ph31
  ...
  br i1 %53, label %.lr.ph26, label %.loopexit

thread-pre-split18:                               ; preds = %.lr.ph26
  %54 = icmp sgt i32 %storemerge6, 1
  br i1 %54, label %.lr.ph26, label %.loopexit

.lr.ph26:                                         ; preds = %49, %thread-pre-split18
  %55 = phi i32 [ %storemerge6, %thread-pre-split18 ], [ %controle, %49 ]
  %56 = and i32 %55, 1
  %57 = icmp eq i32 %56, 0
  %58 = lshr i32 %55, 1
  %59 = mul i32 %55, 3
...
65:                                               ; preds = %.lr.ph26
  ...
  %70 = icmp sgt i32 %controle4, 1
  br i1 %70, label %.lr.ph, label %.loopexit

thread-pre-split20:                               ; preds = %.lr.ph
  %71 = icmp sgt i32 %storemerge, 1
  br i1 %71, label %.lr.ph, label %.loopexit

.lr.ph:                                           ; preds = %65, %thread-pre-split20
  %72 = phi i32 [ %storemerge, %thread-pre-split20 ], [ %controle4, %65 ]
  %73 = and i32 %72, 1
  %74 = icmp eq i32 %73, 0
  %75 = lshr i32 %72, 1
  %76 = mul i32 %72, 3
