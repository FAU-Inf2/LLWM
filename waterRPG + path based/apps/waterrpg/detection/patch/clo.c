...
Bool CLG_(process_cmd_line_option)(const HChar* arg)
{
   ...
   else if VG_BOOL_CLO(arg, "--dump-calls-all",   CLG_(clo).dump_calls_all) {}
   else if VG_STR_CLO(arg, "--dump-calls-objfile", tmp_str) {
     CLG_(clo).dump_calls_objname = VG_(expand_file_name)("--dump-calls-objfile", tmp_str);
   }
...
"    --dump-calls-all=no|yes   Dump every call [no]\n"
"    --dump-calls-objfile=<f>  Only dump calls with callees in objfile <f>\n"
#if CLG_EXPERIMENTAL
...
void CLG_(set_clo_defaults)(void)
{
  ...
  CLG_(clo).dump_calls_all     = False;
  CLG_(clo).dump_calls_objname = 0;
  CLG_(clo).dump_calls_objfile = NULL;
