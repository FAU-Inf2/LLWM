...
obj_node* new_obj_node(DebugInfo* di, obj_node* next)
{
   ...
   if(CLG_(clo).dump_calls_objname != 0 && VG_(strcmp)(obj->name, CLG_(clo).dump_calls_objname) == 0) {
     CLG_ASSERT(CLG_(clo).dump_calls_objfile == NULL);
     CLG_(clo).dump_calls_objfile = obj;
   }
   return obj;
}
