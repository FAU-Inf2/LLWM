...
/* Call File */
static VgFile *call_dump_file = NULL;

/**
 * Create a dump file for calls.
 *
 * Naming: <CLG_(clo).filename_base>.calls
 *
 * Returns the file descriptor or doesn't return at all on error (no write permission)
 */
static VgFile *open_calls_dump(void)
{
    if(call_dump_file != NULL)
      return call_dump_file;
    VgFile *fp = NULL;
    CLG_ASSERT(filename != 0);

    VG_(sprintf)(filename, "%s.calls", out_file);
  ...
  if (fp == NULL) {
    ...
    CLG_DEBUG(2, "  opened call dump '%s'\n", filename);
    call_dump_file = fp;
		return fp;
}

void CLG_(dump_call)(const BBCC *from, const BBCC *to)
{
  VG_(fprintf)(open_calls_dump(), "%u\t%u\t%lx\t%s\t%s\n", from->cxt->fn[0]->number, to->cxt->fn[0]->number, to->bb->offset, to->bb->obj->name, to->cxt->fn[0]->name);
}

/**
 * Close the dump file for calls
 */
static void close_calls_dump(void)
{
    if (call_dump_file == NULL) return;

    VG_(fclose)(call_dump_file);
    call_dump_file = NULL;
}

void CLG_(close_call_dump)(void)
{
  close_calls_dump();
}
